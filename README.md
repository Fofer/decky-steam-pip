...
Steam PiP is a plugin for Decky Loader that gives you flexible, floating picture-in-picture viewing on the Steam Deck and Steam Machine. It floats a small, resizable browser window — a live news stream, a sporting event, a video playing in whatever URL you give it — over whatever you're doing in SteamOS, so you can keep an eye on it without leaving your gameplay. It's also helpful for guides, walkthroughs and tutorials.

Steam PiP works out of the box with YouTube, Twitch, and .m3u8 (HLS) streams from local DVRs (like [Channels DVR](https://getchannels.com/dvr-server/), Plex and HDHomeRun tuners) and IPTV providers — plus any other site or service you can point a browser at. It does not, however, work with copyrighted content protected by DRM such as Widevine (e.g. Netflix, Hulu, Disney+, etc.)

## Features

- **Picture-in-picture or expanded view** — a small floating box you can position in any corner/edge, rezsize or blow up to a large near-fullscreen view, toggled with one button.
- **Works with streaming sites and raw streams** — YouTube, Twitch, and other browser-playable sites work directly; `.m3u8` (HLS) URLs from a local DVR (Channels DVR, Plex, etc.) or an IPTV provider are routed through a bundled HLS.js player so they play instead of triggering a download prompt.
- **Saved channels/bookmarks** — save any number of URLs as named channels, pick them from an organized dropdown, swap between the most recent two, or cycle through them all with a "Channel Surf" up/down control.
- **Playback controls** — play/pause (the button's icon reflects the last command you sent it, not a live readback of the embedded player), jump back/ahead 10 or 30 seconds, volume and mute, all overlaid on the picture without needing to click into the embedded page.
...
## Why it's not available in the Decky Store

The Decky Store's submission process requires confirming that generative AI was not used to write the majority of the code. Most of this fork — including everything added on top of the original `decky-pip` — was written with the help of Claude, an AI assistant, so that box can't be checked honestly. Therefore, it ships as a ZIP install instead.
